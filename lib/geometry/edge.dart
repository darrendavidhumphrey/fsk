...
class Edge {
  ...
  Edge.zero() : start = vm.Vector3.zero(), end = vm.Vector3.zero();

  /// Transforms the edge by a given origin and X and Y axes.
  Edge transform(vm.Vector3 origin3D, vm.Vector3 xAxis, vm.Vector3 yAxis) {
    vm.Vector3 p1Transformed = origin3D + (xAxis * start.x) + (yAxis * start.y);
    vm.Vector3 p2Transformed = origin3D + (xAxis * end.x) + (yAxis * end.y);
  ...
  static List<Edge> transformEdges(
      List<Edge> edges, vm.Vector3 origin3D, vm.Vector3 xAxis, vm.Vector3 yAxis) {
    return edges
  ...
  Edge copyWith({vm.Vector3? start, vm.Vector3? end}) {
    return Edge(
      start ?? this.start,
      end ?? this.end,
    );
